// Steg 2: Microsoft skickar tillbaka Per hit med en engangskod.
//
// Saknar JWT-krav av samma tvingande skal som google-oauth-callback:
// atervandandet ar en webblasarredirect utan var session. Skyddet ar
// state-strangen som ms-oauth-start signerade med tjanstenyckeln -- utan
// giltig signatur, och innan tio minuter gatt, hander ingenting.
...
/** Adressen ur id_token. Signaturen behover inte kontrolleras -- token kom
 *  precis fran Microsofts token-slutpunkt over TLS, inte fran webblasaren. */
function adressUr(idToken: string | undefined): string | null {
  if (!idToken) return null;
  try {
    const p = JSON.parse(franBas64url(idToken.split(".")[1]));
...
Deno.serve(async (req: Request) => {
  ...
  const felText = url.searchParams.get("error_description");

  if (felkod) return tillbaka("fel", "Microsoft nekade: " + (felText ?? felkod));
  if (!kod) return tillbaka("fel", "Microsoft skickade tillbaka dig utan kod");
  ...
  try {
    const svar = await fetch(TOKEN, {
      ...
      body: new URLSearchParams({
        ...
        code: kod,
        redirect_uri: U + "/functions/v1/ms-oauth-callback",
        grant_type: "authorization_code",
      }),
    });
    const json = await svar.json().catch(() => ({}));
    if (!svar.ok || !json.refresh_token) {
      // Utan refresh_token ar anslutningen vardelos -- da har offline_access
      // inte gatt fram, och det ska sagas rakt ut i stallet for att upptackas
      // nasta gang posten ska hamtas.
      const detalj = String(json.error_description ?? json.error ??
        (svar.ok ? "Microsoft gav ett access-token men inget refresh-token" : svar.status)).slice(0, 300);
    ...
    const konto = adressUr(json.id_token);

    const { error } = await admin.rpc("hub_spara_oauth_token", {
      p_user: userId, p_provider: "microsoft", p_token: json.refresh_token, p_konto: konto,
    });
